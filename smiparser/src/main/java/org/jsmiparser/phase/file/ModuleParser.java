...
public class ModuleParser {
    ...
    public IntKeywordToken intkt(Token idToken, SmiPrimitiveType primitiveType,
        ...
        return new IntKeywordToken(makeLocation(idToken), idToken.getText(),
                primitiveType);
    }
    ...
    public BigIntegerToken bintt(Token minusToken, Token t) {
        return new BigIntegerToken(makeLocation(t), minusToken != null,
                t.getText());
    }
    ...
    public void addImports(IdToken moduleToken, List<IdToken> importedTokenList) {
        SmiImports result = new SmiImports(m_module, moduleToken,
                importedTokenList);
        m_module.getImports().add(result);
    }

    public OidComponent createOidComponent(OidComponent parent, Token id,
                                           Token value) {
    ...
    public SmiVariable createVariable(IdToken idToken, SmiType t, Token units,
        ...
        if (units != null) {
            unitsToken = new QuotedStringToken(makeLocation(units),
                    units.getText(), '\"');
        }
    ...
    public SmiTextualConvention createTextualConvention(IdToken idToken,
                                                        Token displayHint, StatusV2 status, Token description,
                                                        Token reference, SmiType type) {
        SmiTextualConvention result = new SmiTextualConvention(idToken,
                m_module, getOptCStr(displayHint), status,
                getCStr(description), getOptCStr(reference));
    ...
    // TODO investigate idea: instead of using the hardcoded SmiConstants here,
    // use SmiReferencedType for everything,
    ...
    public SmiType createIntegerType(IdToken idToken, IntKeywordToken intToken,
                                     Token applicationTagToken, List<SmiNamedNumber> namedNumbers,
                                     List<SmiRange> rangeConstraints) {
        if (idToken == null && intToken.getPrimitiveType() == INTEGER
                && namedNumbers == null && rangeConstraints == null) {
            return SmiConstants.INTEGER_TYPE;
        } else if (idToken != null || namedNumbers != null
                || rangeConstraints != null) {
            SmiType type = createPotentiallyTaggedType(idToken,
                    applicationTagToken);
    ...
    private SmiType createPotentiallyTaggedType(IdToken idToken,
    ...
    public SmiType createOctetStringType(IdToken idToken,
                                         Token applicationTagToken, List<SmiRange> sizeConstraints) {
    ...
    public SmiType createDefinedType(IdToken idToken, Token moduleToken,
                                     Token referencedIdToken, List<SmiNamedNumber> namedNumbers,
                                     List<SmiRange> sizeConstraints, List<SmiRange> rangeConstraints) {
        SmiReferencedType referencedType = new SmiReferencedType(
                idt(referencedIdToken), m_module);
        if (moduleToken != null) {
    ...
    public void addRange(List<SmiRange> rc,
    ...
    public ScopedId makeScopedId(Token moduleToken, Token symbolToken) {
        return new ScopedId(m_module, moduleToken != null ? idt(moduleToken)
                : null, idt(symbolToken));
    }
