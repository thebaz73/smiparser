...
public class ProblemInvocationHandler<T> implements InvocationHandler {
    ...
    public ProblemInvocationHandler(Class<T> cl, ProblemEventHandler eh) {
        ...
        // question: we could always rely on the toString() method to print a
        // value.
        // but then we cannot be sure to get a decent message. Maybe we can
        // check if the
        // class overrides the toString method? If it doesn't, it surely won't
        // give a decent
        // message.

        // TODO check that parameters are not declared as varargs: I don't think
        // we can handle that.
    }
    ...
    class MethodInvocationHandler {
        ...
        public MethodInvocationHandler(Method method) {
            ...
            if (problemMethod == null) {
                throw new IllegalArgumentException(getMethodName(method)
                        + " is missing the "
                        + ProblemMethod.class.getSimpleName() + " annotation");
            }
        ...
        private void check() {
            ...
            for (int i = 0; i < paramAnnotations.length; i++) {
                ...
                if (ep != null) {
                    ...
                    Method m = MethodUtils.getAccessibleMethod(paramType,
                            "getOne" + TextUtil.ucFirst(propertyName),
                            (Class[]) null);
                    if (m == null) {
                        m = MethodUtils.getAccessibleMethod(paramType, "is"
                                + TextUtil.ucFirst(propertyName),
                                (Class[]) null);
                    }
                    if (m == null) {
                        m = MethodUtils.getAccessibleMethod(paramType,
                                propertyName, (Class[]) null);
                    }
                    if (m == null) {
                        throw new IllegalArgumentException(
                                getMethodName(method)
                                        + ": parameter class does not have a property "
                                        + propertyName);
                    }
                }
            }
            ...
            for (int i = 0; i < paramTypes.length; i++) {
                ...
                if (Location.class.equals(c)) {
                    if (locationParameterIndex >= 0) {
                        // throw new
                        // IllegalArgumentException(getMethodName(m_method) +
                        // " has more than one " +
                        // Location.class.getSimpleName() + " parameter");
                    } else {
        ...
        public Object invoke(Object[] args) {
            try {
                ...
                ProblemEvent ev = new ProblemEvent(location,
                        problemMethod.severity(), messageKey,
                        problemMethod.message(), args);
        ...
        private Object[] processParameters(Object[] args)
            ...
            for (int i = 0; i < paramAnnotations.length; i++) {
                ...
                if (ep != null) {
                    Object arg = args[i];
                    String propertyName = ep.value();
                    Method m = MethodUtils.getAccessibleMethod(arg.getClass(),
                            propertyName, (Class[]) null);
                    if (m != null) {
                        args[i] = m.invoke(arg);
                    } else {
