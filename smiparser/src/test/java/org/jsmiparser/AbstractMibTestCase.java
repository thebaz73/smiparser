...
public abstract class AbstractMibTestCase {
    private static final Logger m_log = Logger
            .getLogger(AbstractMibTestCase.class);
    ...
    protected SmiMib getMib() {
        ...
        if (m_mib.get() == null || m_testClass.get() != getClass()) {
            try {
                ...
                stopWatch.stop();
                m_log.info("Parsing time: " + stopWatch.getTotalTimeSeconds()
                        + " s");
    ...
    public SmiType getInteger32() {
        if (m_integer32 == null) {
            ...
            assertSame(SmiPrimitiveType.INTEGER_32,
                    m_integer32.getPrimitiveType());
            assertEquals(1, m_integer32.getRangeConstraints().size());
            assertEquals(-2147483648, m_integer32.getRangeConstraints().get(0)
                    .getMinValue().intValue());
            assertEquals(2147483647, m_integer32.getRangeConstraints().get(0)
    ...
    public SmiType getCounter() {
        if (m_counter == null) {
            ...
            assertSame(SmiPrimitiveType.COUNTER_32,
                    m_counter.getPrimitiveType());
            assertEquals(1, m_counter.getRangeConstraints().size());
            assertEquals(0, m_counter.getRangeConstraints().get(0)
                    .getMinValue().intValue());
            assertEquals(0xFFFFFFFFL, m_counter.getRangeConstraints().get(0)
    ...
    protected void showOverview() {
        for (SmiModule module : m_mib.get().getModules()) {
            for (SmiSymbol symbol : module.getSymbols()) {
                String msg = module.getId() + ": " + symbol.getId() + ": "
    ...
    protected void checkOidTree(SmiMib mib) {
        ...
        for (SmiSymbol symbol : mib.getSymbols()) {
            if (symbol instanceof SmiOidValue) {
                ...
                if (oidValue.getNode() != mib.getRootNode()) {
                    ...
                    SmiOidNode foundOidNode = oidValue.getNode().getParent()
                            .findChild(oidValue.getNode().getValue());
                    assertNotNull(msg, foundOidNode);
                    assertSame(msg, oidValue.getNode(), foundOidNode);
                    assertTrue(
                            msg,
                            oidValue.getNode().getParent()
                                    .contains(oidValue.getNode()));
                }
                // SmiOidValue foundSymbol = findOidSymbol(mib.getRootNode(),
                // symbol.getId());
        ...
        // - due to anonymous oid node, you can have more oid nodes than oid
        // symbols
